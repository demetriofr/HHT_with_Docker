...
1. Клонируйте содержимое данного репозитория к себе: `git clone git@github.com:demetriofr/healthy_habits_tracker.git`                                                                                
2. Установите зависимости в venv: `pip3 install -r requirements.txt`.                                                               
3. Создайте базу данных, например, через консоль:                                                                                   
   ```                                                                                                                                 
   psql -U postgres                                                                                                                    
   create database hht_with_docker;
   \q                                                                                                
   ```                                                                                                                                 
...
8. Запустите в отдельных терминалах кэширование с помощью redis: `redis-server`.                                                                                           
9. Запустите в отдельных терминалах периодические задачи:
...
### Описание задач
- [ ] Cоздайть в проекте отдельные контейнеры для следующих сервисов:
- - Django,
- - PostrgeSQL,
- - Redis,
- - Celery.
...
- [ ] Всё оформлено в файле docker-compose и создан вспомогательный Dockerfiles.
...
- [ ] Для выполнения задания используется курсовой проект из курса DRF.                                                 
